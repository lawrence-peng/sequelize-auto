  ...
  function generateText (err) {
    ...
    async.each(_.keys(self.tables), function (table, _callback) {
      ...
      _.each(fields, function (field, i) {
        ...
        fieldStr += spaces + spaces + fieldName + ': {\n'
        ...
        _.each(fieldAttr, function (attr, x) {
          ...
          if (attr === 'foreignKey') {
            if (isSerialKey) {
              fieldStr += spaces + spaces + spaces + 'autoIncrement: true'
            }
            else if (foreignKey.isForeignKey) {
              fieldStr += spaces + spaces + spaces + 'references: {\n'
              fieldStr += spaces + spaces + spaces + spaces + 'model: \'' + self.tables[table][field][attr].foreignSources.target_table + '\',\n'
              fieldStr += spaces + spaces + spaces + spaces + 'key: \'' + self.tables[table][field][attr].foreignSources.target_column + '\'\n'
              fieldStr += spaces + spaces + spaces + '}'
            ...
            if (self.tables[table][field][attr] === true && (!_.has(self.tables[table][field], 'foreignKey') || (_.has(self.tables[table][field], 'foreignKey') && !!self.tables[table][field].foreignKey.isPrimaryKey)))
              fieldStr += spaces + spaces + spaces + 'primaryKey: true'
            else return true
          }
          else if (attr === 'allowNull') {
            fieldStr += spaces + spaces + spaces + attr + ': ' + self.tables[table][field][attr]
          }
            ...
            if (defaultVal === null || defaultVal === undefined) {
              ...
              fieldStr += spaces + spaces + spaces + attr + ': ' + val_text
            }
          }
          else if (attr === 'type' && self.tables[table][field][attr].indexOf('ENUM') === 0) {
            fieldStr += spaces + spaces + spaces + attr + ': DataTypes.' + self.tables[table][field][attr]
          }
          else {
            ...
            fieldStr += spaces + spaces + spaces + attr + ': ' + val
          }
        ...
        if (isUnique) {
          fieldStr += spaces + spaces + spaces + 'unique: true,\n'
        }

        if (self.options.camelCase) {
          fieldStr += spaces + spaces + spaces + 'field: \'' + field + '\',\n'
        }

        // removes the last `,` within the attribute options
        fieldStr = fieldStr.trim().replace(/,+$/, '') + '\n'

        fieldStr += spaces + spaces + '}'
      ...
      optionsStr += spaces + spaces + 'tableName: \'' + table + '\',\n'
      ...
      function addAdditionalOption (value, key) {
        if (key === 'name') {
          ...
          optionsStr += spaces + spaces + 'name: {\n'
          optionsStr += spaces + spaces + spaces + 'singular: \'' + table + '\',\n'
          optionsStr += spaces + spaces + spaces + 'plural: \'' + table + '\'\n'
          optionsStr += spaces + spaces + '},\n'
        }
        else {
          value = _.isBoolean(value) ? value : ('\'' + value + '\'')
          optionsStr += spaces + spaces + key + ': ' + value + ',\n'
        }
      }

      text[table] += fieldStr + ',' + optionsStr

      //resume normal output
      text[table] += ');\n};\n'
      _callback(null)

      tables.push({tableName: tableName, fields: fieldStr, options: optionsStr})
      ...
      if (self.options.directory) {
        if (self.options.template) {
          return self.writeByTemplate(self.options.template, tables, callback)
        }
...
AutoSequelize.prototype.writeByTemplate = function (templateInfo, data, callback) {
  var templateKeys = _.keys(templateInfo)
  async.each(templateKeys, function (key) {
    var temp = templateInfo[key]
    if (temp.use === false) {
      return
    }
    mkdirp.sync(path.resolve(temp.directory))
    var modelTemplate = require(path.resolve(temp.templatePath))
    var template = Handlebars.compile(modelTemplate)
    async.each(data, createFile, callback)

    function createFile (item, _callback) {
      var result = template(item)
      fs.writeFile(path.resolve(path.join(temp.directory, item.tableName + '.js')), result, _callback)
    }
  })
}
